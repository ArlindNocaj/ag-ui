...
/**
 * `generate_task_steps` is a FRONTEND tool (RunAgentInput.tools). The runtime
 * suspends the call, the run finishes, the browser renders the step picker, and
 * the next run resolves the original call with the user's selection.
 */
export const createHumanInTheLoopAgent = (client: CopilotClientPort) =>
  defineAgent(client, {
    agentId: "human_in_the_loop",
    description: "Task planner that waits for human approval",
