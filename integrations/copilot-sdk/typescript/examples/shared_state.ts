...
const recipeSchema = {
  ...
  properties: {
    ...
    cooking_time: { type: "string", enum: ["5 min", "15 min", "30 min", "45 min", "60+ min"] },
    ingredients: {
      type: "array",
      description: "Entire list of ingredients, existing and new: icon (emoji like 🥕), name and amount",
      items: {
        type: "object",
        properties: { icon: { type: "string" }, name: { type: "string" }, amount: { type: "string" } },
  ...
  required: ["skill_level", "special_preferences", "cooking_time", "ingredients", "instructions"],
};

/** The tool's argument IS the new state: the handler snapshots it for the UI. */
export const createSharedStateAgent = (client: CopilotClientPort) =>
  defineAgent(client, {
    agentId: "shared_state",
    description: "Recipe assistant with bidirectional shared state",
    ...
    tools: [
      {
        ...
        parameters: { type: "object", properties: { recipe: recipeSchema }, required: ["recipe"] },
        handler: (args: { recipe: unknown }, ctx) => {
          ctx.setState({ recipe: args.recipe });
